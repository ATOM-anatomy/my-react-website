      ...
      <header className="sticky top-0 z-50 backdrop-blur bg-slate-900/60 border-b border-slate-800">
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <nav className="hidden md:flex gap-6 text-sm">
            {sections.map(s => (
              <a key={s.id} href={`#${s.id}`} className="hover:text-white text-slate-300 transition">
                {s.label}
              </a>
            ))}
          </nav>
        ...
        {menuOpen && (
          <div className="md:hidden border-t border-slate-800 px-4 pb-3 bg-slate-900/80">
            <div className="flex flex-col gap-2 pt-2">
              {sections.map(s => (
                <a key={s.id} href={`#${s.id}`} className="py-2 text-slate-200" onClick={() => setMenuOpen(false)}>
                  {s.label}
                </a>
              ))}
            </div>
      ...
      <section id="top" className="relative overflow-hidden">
        <div className="max-w-6xl mx-auto px-4 py-20">
          ...
          <motion.div {...fadeUp} className="mt-10 grid md:grid-cols-2 gap-6">
            <img className="rounded-2xl ring-1 ring-white/10 shadow-xl object-cover h-64 w-full" src="https://images.unsplash.com/photo-1518779578993-ec3579fee39f?auto=format&fit=crop&w=1600&q=80" alt="วงจรรวมและชิป"/>
            <img className="rounded-2xl ring-1 ring-white/10 shadow-xl object-cover h-64 w-full" src="https://images.unsplash.com/photo-1519389950473-47ba0277781c?auto=format&fit=crop&w=1600&q=80" alt="ทีมงานกับจอภาพข้อมูลอนาคต"/>
          </motion.div>
        </div>
      </section>
          ...
          <div className="mt-6 grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {[
              ...
              <figure key={i} className="relative group overflow-hidden rounded-2xl ring-1 ring-white/10">
                <img src={img.src} alt={img.alt} className="h-56 w-full object-cover transition group-hover:scale-105"/>
